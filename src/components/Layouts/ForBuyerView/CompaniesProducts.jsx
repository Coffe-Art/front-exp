...
import { useNavigate, useParams } from 'react-router-dom'; // Agregar useParams para obtener parámetros de URL
...
export const CompaniesProducts = () => {
  ...
  const [loadingMessage, setLoadingMessage] = useState(''); // Agregado para manejar el mensaje de carga

  const navigate = useNavigate();
  const { codigoempresa } = useParams(); // Obtener el ID de la empresa desde los parámetros de la URL

  const fetchProductos = async (codigoempresa) => {
    setCargando(true); // Establece cargando en true al comenzar la solicitud
    ...
    try {
      ...
      setCargando(false); // Establece cargando en false después de la solicitud
      setLoadingMessage('');
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {productosFiltrados.map(producto => (
                ...
                >
                  ...
                  />
                  <h3 className="text-2xl font-semibold mt-5">{producto.nombreProducto}</h3>
...
>
  Agregar al carrito
      ...
      <h3 className="text-xl font-bold mb-4 ">{productoSeleccionado.nombreProducto}</h3>
      </div>
            <img
        src={
